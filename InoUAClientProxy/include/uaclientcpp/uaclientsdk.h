...
#if OPCUA_SUPPORT_PKI && OPCUA_SUPPORT_PKI_WIN32
...
public:
    ...
    */
    ...
    /** @brief The locale ID requested for the session.
    *
    * Examples are
    * - "en" for English
    ...
    */
    ...
    /** @brief Additional locale IDs to be requested for the session.
    ...
    */
    ...
    /** @brief A flag indicating if the client SDK should wait for an reverse TCP/IP connect from the server
     *
     * If this flag is OpcUa_True, the sClientEndpointUrl is used to open a listen port to allow the server to do an reverse
     * TCP/IP connect to the client. After the reverse TCP/IP connect, the rest of the connection establishment is done like for the
     * normal connect.
     *
     * The default value is OpcUa_False.
     */
    OpcUa_Boolean  bIsReverseConnect;

    /** @brief Client side endpoint URL used to open a listen port to allow the server to do an reverse connect to the client
     *
     * After the reverse TCP/IP connect, the rest of the connection establishment with secure channel and session creation
     * is done like for the normal connect.
     */
    UaString       sClientEndpointUrl;
...
public:
    ...
    UaStatusCode loadClientCertificateOpenSSL(
        ...
        const UaString& sPassword = UaString());
...
#endif // OPCUA_SUPPORT_PKI && OPCUA_SUPPORT_PKI_WIN32
    ...
    OpcUa_Key*                getClientPrivateKey();
    UaByteArray               getClientPrivateKeyDer();
    void                      setClientPrivateKeyDer(const UaByteString& privateKey);
    ...
    * OpcUa_SecurityPolicy_Aes128Sha256RsaOaep | "http://opcfoundation.org/UA/SecurityPolicy#Aes128_Sha256_RsaOaep"
    * OpcUa_SecurityPolicy_Aes256Sha256RsaPss  | "http://opcfoundation.org/UA/SecurityPolicy#Aes256_Sha256_RsaPss"
    */
...
public:
    ...
    OpcUa_UserTokenType userTokenType() const;
    ...
    */
    ...
    The check will fail if either the ServerNonce is not set or the PasswordEncryptionMode is None. The default is false. */
    ...
    /** \brief Flag used to disable the check if the certificate used to encrypt a password needs to be trusted.
    The check will fail if the server certificate used to encrypt a user token is not trusted. The default is false. */
    OpcUa_Boolean             disableTrustedCertificateForUserTokenRequired;
};
