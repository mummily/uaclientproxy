...
#if OPCUA_LISTENER_SUPPORT_CONNECT
    /** @brief A secure channel has been created. */
    eOpcUa_Endpoint_Event_SecureChannelPending,
 ...
 * @param sServerUri                [in] The URI of the server. (String must be zeroterminated.)
...
#if OPCUA_LISTENER_SUPPORT_CONNECT
/**
 * @brief Actively connect from an open endpoint to a passive client.
 * Endpoint needs to be opened to use this function.
 * 
 * @param hEndpoint            [in] The endpoint handle (endpoint must be open).
 * @param sUrl                 [in] URL with hostname/IP and port of the passive client.
 * @param sTransportProfileUri [in] Specifies the transport profile URI, i.e. OpcUa_TransportProfile_UaTcp. (String must be zeroterminated.)
 * @return OpcUa_True if endpoint is closed; OpcUa_False otherwise.
 */
OPCUA_EXPORT
OpcUa_StatusCode OpcUa_Endpoint_BeginConnect(
    OpcUa_Endpoint      hEndpoint,
    const OpcUa_CharA*  sUrl,
    const OpcUa_CharA*  sTransportProfileUri);
#endif /* OPCUA_LISTENER_SUPPORT_CONNECT */
