...
public:
    ...
    *
    * The default implementation returns NA (No capability information is available)
    *
    *  @return                     Error code.
    */
    virtual UaStatus getServerCapabilities(
        UaStringArray& serverCapabilityArray         //!< [out] Array of capability identifiers indicating the features supported by the server.
        ) const;
    ...
    */
    virtual UaStatus getDurableSubscriptionSettings(
        OpcUa_Boolean& durableSubscriptionEnabled,     //!< [out] Flag indicating if durable subscription support is enabled
        UaString& storageDirectory,                    //!< [out] File directory used by the SDK to store durable subscriptions
        OpcUa_UInt32& iMaxSubscriptionLifetimeInHours, //!< [out] Maximum Durable Subscription lifetime in hours the server allows
        OpcUa_UInt32& iMaxMonitoredItemQueueSize,      //!< [out] Maximum size of a durable monitored item queue the server allows
        OpcUa_UInt32& iLiveQueueSize                   /**< [out] Size of the live value queue for a durable monitored item in normal
                                                                  operation. The default value is 100. The minimum value is 10.*/
        ) const = 0;
